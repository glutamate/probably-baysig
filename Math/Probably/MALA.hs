...
mala1 :: Matrix Double -> (forall a. (Real a, Floating a) => [a]->a) 
         -> MalaPar
         -> Sampler MalaPar
mala1 cov pdf (MalaPar xi pi mbgrad sigma tr tracc freeze) = do
...
runMala cov pdf nsam init = go nsam mp1 [] where
  ...
  pi = pdf $ toList $ init
  mp1 =  MalaPar init pi Nothing 1 0 0 False
...
runMalaMPaccept :: Matrix Double -> (forall a. (Real a, Floating a) => [a]->a) 
         ->  Int -> MalaPar -> [(Double,Vector Double)] -> RIO (MalaPar, [(Double,Vector Double)])
runMalaMPaccept  cov pdf nsam init xs0 = go init xs0 where
  go mpar xs 
...
runMalaRioESS cov pdf want_ess xi = do
    ...
    let mp0 = MalaPar xi p0 Nothing sigma0 0 0 False
        nsam0 = want_ess*20
    ...
    let go mp n xs = do
            ...
            if  mannWhitneyUtest TwoTailed 0.10 (U.fromList $ map fst xs2)
               ...
               then do io$ putStrLn "not converged"
                       go mp2 (round $ realToFrac n*1.5) xs2
